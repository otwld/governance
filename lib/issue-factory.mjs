...
function validProjectIssue(item, project, repository, issueUrl) {
  return item?.type === 'ISSUE' && item?.isArchived === false && item?.content?.__typename === 'Issue' && item.content.url === issueUrl && item.content.repository?.nameWithOwner?.toLowerCase() === repository.toLowerCase() && projectSingleSelect(item, project.statusFieldId) !== undefined;
}
