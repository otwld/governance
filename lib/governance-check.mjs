...
export function normalizeProjectItems(projectConfig, nodes) {
  ...
  const items = nodes.map((item, index) => {
    ...
    /** Missing Priority is policy-controlled, but duplicate or malformed field evidence is ambiguous. */
    const priorityValues = Array.isArray(item?.fieldValues?.nodes) ? item.fieldValues.nodes.filter((value) => value?.field?.id === project.priorityFieldId) : [];
    ...
    if (priorityValues.length > 1) diagnostics.push(`items[${index}].priority: ambiguous configured priority field`);
    else if (priorityValues.length === 1 && !priority) diagnostics.push(`items[${index}].priority: invalid configured priority field`);
...
export function selectDeterministicQueue(projectConfig, items) {
  ...
  for (const [index, item] of items.entries()) {
    ...
    if (item.repository?.toLowerCase?.() !== projectConfig.repository.toLowerCase() || !issueIdentity(item.issueUrl, projectConfig.repository)) diagnostics.push(`${path}.issueUrl: must belong to project repository`);
    eligible.push(item);
  }
