...
function hasExplicitDefaultDeny(lines) {
  ...
  return block.some((line) => {
    const match = /^(\s+)(?:"\*"|'\*'|\*):\s*deny\s*(?:#.*)?$/.exec(line);
    return match !== null && match[1].length === topLevelIndent;
  });
}
...
export async function validateDistribution(
  ...
  if (requireProductionAssets) {
    diagnostics.push(
      ...validateProductionNames(
        'agents',
        'agent',
        agentFiles.map((file) => displayPath(agentsRoot, file).slice(0, -3)),
        PRODUCTION_AGENTS,
      ),
    );
  }
  for (const file of agentFiles) {
    ...
    if (!hasExplicitDefaultDeny(parsed.lines)) {
      diagnostics.push(
        diagnostic(shown, 'permission must be a block with explicit top-level "*" set to deny'),
      );
    }
  }
