...
const PRODUCTION_AGENTS = ['brainstormer', 'orchestrator', 'implementer', 'reviewer', 'researcher', 'task-shaper'];
const PRODUCTION_COMMANDS = ['brainstorm', 'orchestrate', 'orchestrate-loop', 'setup-project', 'review', 'shape-task'];
const PRODUCTION_TOOLS = ['create_issue'];
...
const PRODUCTION_AGENT_DEFAULT_PERMISSIONS = new Map([
  ['brainstormer', 'deny'],
  ['orchestrator', 'allow'],
  ['implementer', 'allow'],
  ['reviewer', 'allow'],
...
const PRODUCTION_ISSUE_FIELDS = new Map([
  ['outcome', true],
...
function explicitDefaultPermissionAction(lines) {
  return explicitNamedPermissionAction(lines, '*');
}

function explicitNamedPermissionAction(lines, name) {
  ...
  for (const line of block) {
    const match = new RegExp(
      `^(\\s+)(?:"${escapedName}"|'${escapedName}'|${escapedName}):\\s*(allow|deny)\\s*(?:#.*)?$`,
    ).exec(line);
...
async function validateProductionTools(root) {
  ...
  try {
    ...
    if (error.code === 'ENOENT') {
      return validateProductionNames('tools', 'tool', [], PRODUCTION_TOOLS);
    }
    return [diagnostic('tools', `cannot read directory: ${error.message}`)];
  }

  const toolNames = entries
    .filter((entry) => entry.isFile() && entry.name.endsWith('.js'))
    .map((entry) => entry.name.slice(0, -3));
  const diagnostics = validateProductionNames('tools', 'tool', toolNames, PRODUCTION_TOOLS);
  for (const name of PRODUCTION_TOOLS) {
    if (!toolNames.includes(name)) continue;
    const shown = `tools/${name}.js`;
    try {
      if ((await readFile(join(toolsRoot, `${name}.js`), 'utf8')).trim() === '') {
        diagnostics.push(diagnostic(shown, 'production tool source must not be empty'));
      }
    } catch (error) {
      diagnostics.push(diagnostic(shown, `cannot read file: ${error.message}`));
    }
  }
  return diagnostics;
}

function effectiveToolPermission(globalPermission, agentPermission, tool) {
  let action;
  if (typeof globalPermission === 'string') {
    action = globalPermission;
  } else if (isRecord(globalPermission)) {
    if (typeof globalPermission['*'] === 'string') action = globalPermission['*'];
    if (typeof globalPermission[tool] === 'string') action = globalPermission[tool];
  }
  if (agentPermission.defaultAction !== undefined) action = agentPermission.defaultAction;
  if (agentPermission.toolAction !== undefined) action = agentPermission.toolAction;
  return action;
}
...
async function validateTemplate(
  ...
  } else if (requireProductionAssets && defaultAgent !== 'orchestrator') {
    diagnostics.push(
      diagnostic(`${shown}.default_agent`, 'production default agent must be "orchestrator"'),
    );
  }

  if (
    requireProductionAssets &&
    (!isRecord(config.permission) || config.permission.create_issue !== 'deny')
  ) {
    diagnostics.push(
      diagnostic(
        `${shown}.permission.create_issue`,
        'production template must set global create_issue permission to deny',
      ),
    );
  }

  if (requireProductionAssets && PRODUCTION_AGENTS.every((name) => agents.has(name))) {
    for (const name of PRODUCTION_AGENTS) {
      const permission = agentPermissions.get(name) ?? {};
      const expected = name === 'task-shaper' ? 'allow' : 'deny';
      const effective = effectiveToolPermission(config.permission, permission, 'create_issue');
      if (permission.toolAction !== expected || effective !== expected) {
        diagnostics.push(
          diagnostic(
            `agents/${name}.md`,
            `production agent "${name}" must explicitly set create_issue permission to ${expected} and resolve it to ${expected}`,
          ),
        );
      }
    }
...
async function validateProductionIssueTemplate(root) {
  const shown = 'templates/.github/ISSUE_TEMPLATE/agent-task.yml';
  let content;
  try {
    content = await readFile(join(root, shown), 'utf8');
  } catch (error) {
    const reason = error.code === 'ENOENT' ? 'file does not exist' : `cannot read file: ${error.message}`;
    return [diagnostic(shown, reason)];
  }

  const diagnostics = [];
  const title = /^title:\s*(.*)$/m.exec(content);
  if (title !== null && unquoteYamlScalar(title[1]) !== '') {
    diagnostics.push(diagnostic(`${shown}.title`, 'must not define an automatic title prefix'));
  }

  const lines = content.replace(/^\uFEFF/, '').split(/\r?\n/);
  const bodyDefinitions = lines
    .map((line, index) => ({ index, match: /^body\s*:(.*)$/.exec(line) }))
    .filter(({ match }) => match !== null);
  let bodyStart = -1;
  let bodyEnd = -1;
  if (bodyDefinitions.length !== 1) {
    diagnostics.push(diagnostic(`${shown}.body`, 'must define exactly one top-level "body" key'));
  } else {
    const [{ index, match }] = bodyDefinitions;
    if (!/^\s*(?:#.*)?$/.test(match[1])) {
      diagnostics.push(diagnostic(`${shown}.body`, 'must be a block sequence'));
    } else {
      bodyStart = index + 1;
      bodyEnd = bodyStart;
      ...
      if (!lines.slice(bodyStart, bodyEnd).some((line) => /^  -(?:\s|$)/.test(line))) {
        diagnostics.push(diagnostic(`${shown}.body`, 'must be a block sequence'));
      }
    }
  }

  const fieldLikeOutsideBody = lines.some((line, index) =>
    /^  - type:\s*\S+\s*$/.test(line) && !(index >= bodyStart && index < bodyEnd),
  );
  if (fieldLikeOutsideBody) {
    diagnostics.push(diagnostic(`${shown}.body`, 'contract fields must be inside the top-level "body" sequence'));
  }

  const fields = [];
  for (let index = bodyStart; index >= 0 && index < bodyEnd; index += 1) {
    const typeLine = /^  - type:\s*(\S+)\s*$/.exec(lines[index]);
    if (typeLine === null) continue;
    let end = index + 1;
    while (end < bodyEnd && !/^  - type:/.test(lines[end])) end += 1;
    const block = lines.slice(index + 1, end);
    const idLine = block.find((line) => /^    id:\s*\S/.test(line));
    if (idLine === undefined) continue;
    const id = unquoteYamlScalar(idLine.replace(/^    id:\s*/, ''));
    const validations = block.findIndex((line) => /^    validations:\s*$/.test(line));
    const requiredLine = validations === -1
      ? undefined
      : block.slice(validations + 1).find((line) => /^      required:\s*(?:true|false)\s*$/.test(line));
    fields.push({
      id,
      type: unquoteYamlScalar(typeLine[1]),
      required: requiredLine === undefined
        ? false
        : requiredLine.replace(/^      required:\s*/, '').trim() === 'true',
    });
    index = end - 1;
  }
  ...
  for (const { id } of fields) {
    if (seen.has(id)) {
      diagnostics.push(diagnostic(`${shown}.body.${id}`, 'field id must be unique'));
    }
    seen.add(id);
  }
  for (const [id, required] of PRODUCTION_ISSUE_FIELDS) {
    const matches = fields.filter((field) => field.id === id);
    if (matches.length === 0) {
      diagnostics.push(diagnostic(`${shown}.body`, `missing required contract field "${id}"`));
    } else if (matches[0].type !== 'textarea') {
      diagnostics.push(diagnostic(`${shown}.body.${id}`, 'field must use type "textarea"'));
    } else if (matches[0].required !== required) {
      diagnostics.push(
        diagnostic(
          `${shown}.body.${id}`,
          `field must be ${required ? 'required' : 'optional'}`,
        ),
      );
    }
  }
  for (const { id } of fields) {
    if (!PRODUCTION_ISSUE_FIELDS.has(id)) {
      diagnostics.push(diagnostic(`${shown}.body.${id}`, 'unexpected production contract field'));
    }
  }
  return diagnostics;
}
...
export async function validateDistribution(
  ...
  for (const file of agentFiles) {
    ...
    agentPermissions.set(name, {
      defaultAction: defaultPermissionAction,
      toolAction: explicitNamedPermissionAction(parsed.lines, 'create_issue'),
    });
    ...
    const expectedDefaultPermission = requireProductionAssets
      ? PRODUCTION_AGENT_DEFAULT_PERMISSIONS.get(name)
      : undefined;
    if (
      hasAllProductionAgents &&
      expectedDefaultPermission !== undefined &&
      defaultPermissionAction !== expectedDefaultPermission
    ) {
      diagnostics.push(
        diagnostic(
          shown,
          `production agent "${name}" must set top-level "*" permission to ${expectedDefaultPermission}`,
        ),
      );
    }
  ...
  if (requireProductionAssets) {
    diagnostics.push(...(await validateProductionTools(absoluteRoot)));
  }
  diagnostics.push(...(await validateSkills(absoluteRoot, { requireProductionAssets })));
  diagnostics.push(
    ...(await validateTemplate(absoluteRoot, agents, agentPermissions, { requireProductionAssets })),
  );
  if (requireProductionAssets) {
    diagnostics.push(...(await validateProductionIssueTemplate(absoluteRoot)));
  }
  return diagnostics;
}
