...
/** Distributed agents use only the audited GPT-5.6 model tiers. */
const agentModels = new Set(['openai/gpt-5.6-luna', 'openai/gpt-5.6-terra', 'openai/gpt-5.6-sol']);
/** Reasoning settings remain within the variants exposed by the configured OpenAI provider. */
...
/** Validate the installed asset graph, declared custom-tool ownership, schemas, and manifest ownership. */
export async function validateDistribution(root = process.cwd()) {
  ...
  for (const [name, value] of Object.entries(manifest.agents)) {
    if (!record(value) || !['primary', 'subagent'].includes(value.mode)) diagnostics.push(diagnostic(`governance.manifest.json.agents.${name}`, 'must declare a primary or subagent mode'));
    if (!agentModels.has(value?.model)) diagnostics.push(diagnostic(`governance.manifest.json.agents.${name}.model`, 'must declare an audited GPT-5.6 model'));
    if (!reasoningEfforts.has(value?.reasoningEffort)) diagnostics.push(diagnostic(`governance.manifest.json.agents.${name}.reasoningEffort`, 'must declare a supported reasoning effort'));
  }
  ...
  for (const name of agentRoster) {
    ...
    if (parsed.data.model !== manifest.agents[name]?.model) diagnostics.push(diagnostic(path, `model must be ${manifest.agents[name]?.model}`));
    if (parsed.data.reasoningEffort !== manifest.agents[name]?.reasoningEffort) diagnostics.push(diagnostic(path, `reasoningEffort must be ${manifest.agents[name]?.reasoningEffort}`));
    if (permission(parsed.lines) !== 'allow') diagnostics.push(diagnostic(path, 'development agents must allow ordinary tools by default'));
