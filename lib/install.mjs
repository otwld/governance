...
async function addTopLevelFiles(sourceDirectory, destinationDirectory, extension, files) {
...
async function buildLayout({ sourceRoot, configHome, skillsHome }) {
  ...
  const toolHome = join(configHome, 'tools');
  const directories = new Set([agentHome, commandHome, toolHome, skillsHome]);

  await addTopLevelFiles(join(sourceRoot, 'agents'), agentHome, '.md', files);
  await addTopLevelFiles(join(sourceRoot, 'commands'), commandHome, '.md', files);
  await addTopLevelFiles(join(sourceRoot, 'tools'), toolHome, '.js', files);
